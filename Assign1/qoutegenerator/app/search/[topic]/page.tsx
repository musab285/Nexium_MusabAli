...
export default function TopicPage() {
    ...
    return (
        <div className="bg-hero bg-gray-700 p-6 min-h-screen font-mono justify-center w-full mx-auto bg-gray-200">
            <div className="text-4xl text-shadow-lg/20 font-semibold text-center bg-linear-to-r from-transparent via-green-100/85 to-transparent rounded-xl mx-10 p-2 capitalize ">Quotes on {topic.charAt(0).toUpperCase() + topic.slice(1)}{author && " by "}{(author? author:"")}...</div>
            <div className="flex flex-col justify-center h-[calc(100vh-20vh))]">
                <ScrollArea className="rounded-xl max-h-[calc(100vh-24vh)] border-collapse w-auto mx-2 px-6 py-2">
                    {quotes.map((quote) => (
                        <Card
                            className="my-6 dropshadow-2xl border-2 bg-green-100/85 rounded-xl border-2 shadow-gray-400 mx-2 hover:-translate-y-1 hover:cursor-pointer hover:shadow-md delay-150 duration-500"
                            key={quote.id}
                        >
                            <CardHeader>
                                <CardTitle className="font-bold text-md/20 text-shadow-md">{quote.author}~</CardTitle>
                            </CardHeader>
                            <CardContent >
                                <p>{quote.text}</p>
                            </CardContent>
